...
const Rmtdt = () => {
    ...
    const selectItem = (id) => {
        ...
        setSelectedItems([...selectedItems, {...item, measure:'RMTDT', startDate:null, endDate:null}]);
    }
    ...
    const handleGraphs = (items) => {
        // items.map(item => {
        //     sendLiveDataCommand(item.socketId, "startLiveData", item);
        // });
    ...
    const verificationRMTDT = (items,timeInterval=false) => {
        let result = true;
        if (timeInterval){
            items.map(item => {
                if (
        ...
        <Dashboard>
            <div className={styles.mainContainer}>
                ...
                <ActionBlock
                    ...
                    blockButton={selectedItems.length!=0 ? verificationRMTDT(selectedItems,withTime): false}
                ...
                { openGraph &&
                    <div className={styles.graphContainer}>
                        <div className={styles.graphContent}>
                            ...
                            {/* {items.filter(item => selectedItems.includes(item.id)).map((item, index) => (
